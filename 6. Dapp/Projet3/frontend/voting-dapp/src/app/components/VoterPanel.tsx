  ...
  if (!contract) {
    return (
      <div className="bg-white shadow-lg rounded-xl p-8 border border-gray-100">
        <h2 className="text-2xl font-bold text-gray-800 mb-6">Espace Électeur</h2>
        <button 
  ...
  if (!isRegistered) {
    return (
      <div className="bg-gradient-to-br from-amber-50 to-orange-50 border-2 border-amber-200 shadow-lg rounded-xl p-8">
        <h2 className="text-2xl font-bold text-amber-800 mb-4">Espace Électeur</h2>
        <div className="flex items-center mb-4">
          ...
          <p className="text-amber-700 font-medium">Vous n'êtes pas inscrit pour voter</p>
        </div>
    ...
    <div className="bg-white shadow-lg rounded-xl p-8 border border-gray-100">
      ...
      {workflowStatus === 1 && (
        <div className="mb-8 p-6 bg-gradient-to-br from-blue-50 to-indigo-50 border-2 border-blue-200 rounded-xl">
          <h3 className="text-lg font-bold text-blue-800 mb-4">📝 Soumettre une proposition</h3>
      ...
      {workflowStatus === 3 && !hasVoted && proposals.length > 0 && (
        <div className="mb-8 p-6 bg-gradient-to-br from-green-50 to-emerald-50 border-2 border-green-300 rounded-xl">
          <h3 className="text-lg font-bold text-green-800 mb-4">🗳️ Session de vote active</h3>
          <p className="text-green-700 mb-4">Choisissez la proposition pour laquelle vous souhaitez voter :</p>
          <div className="space-y-4">
            {proposals.map((proposal, index) => (
              <div key={index} className="bg-white border-2 border-green-200 rounded-lg p-4 hover:border-green-300 transition-colors duration-200">
                <div className="mb-3">
                  <h4 className="font-bold text-gray-800 mb-1">Proposition #{index}</h4>
                ...
                >
                  {loading ? 'Vote en cours...' : '✓ Voter pour cette proposition'}
                </button>
              </div>
            ))}
          </div>
      ...
      {workflowStatus === 3 && hasVoted && (
        <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
          <div className="flex items-center">
            ...
            <p className="text-blue-800 font-medium">Votre vote a été enregistré avec succès</p>
          </div>
        </div>
      )}
      ...
      {workflowStatus === 3 && !hasVoted && proposals.length === 0 && (
        <div className="mb-6 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
          <div className="flex items-center">
            ...
            <p className="text-yellow-800">Aucune proposition disponible pour le vote</p>
          </div>
        </div>
      )}
          ...
          <div className="space-y-3">
            {proposals.map((proposal, index) => (
              <div key={index} className="bg-slate-50 border border-slate-200 rounded-lg p-4">
                <h4 className="font-bold text-gray-800 text-sm mb-2">Proposition #{index}</h4>
                <p className="text-gray-700 text-sm leading-relaxed">{proposal.description}</p>
                {workflowStatus >= 4 && (
                  <div className="mt-2 text-xs text-slate-600">
                    <span className="font-medium">Votes reçus: {proposal.voteCount?.toString() || '0'}</span>
                  </div>
                )}
              </div>
            ))}
