...
contract Voting is Ownable {
    ...
    function addProposal(string calldata _desc) external onlyVoters {
        ...
        // Vérifie si le nombre maximum de propositions n'est pas dépassé
        require(proposalsArray.length < MAX_PROPOSALS, "Maximum proposals reached");
    ...
    function tallyVotes() external onlyOwner {
        ...
        uint256 winningVoteCount = proposalsArray[0].voteCount;

        // Parcourt toutes les propositions pour trouver celle avec le plus de votes
        for (uint256 p = 1; p < proposalsArray.length; p++) {
            uint256 currentVoteCount = proposalsArray[p].voteCount;
